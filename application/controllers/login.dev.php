...
class Login extends CI_Controller {
	...
	public function sendmycode(){
			...
			if($this->input->post()){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$table = "users";
					$account = $this->model_login->login($login_data, $table);
					if( count($account) >= 1){
						$tbname = 'users';
						$col = 'user_id';
						$data_update = array( 
						...
						$this->model_base->update_data($account[0]['user_id'],$col,$data_update,$tbname);
	...
	public function expirycode(){
		$accounts = $this->model_base->get_all('users');
			
		foreach ($accounts as $account){
			if(!empty($account['user_code']) && !empty($account['user_code_time'])){
					if($account['user_code_time'] >= 2){
						$tbname = 'users';
						$col = 'user_id';
						$count_down = $account['user_code_time'] - 1;
						$data_update = array( 
							'user_code_time' => $count_down
						);
						$this->model_base->update_data($account['user_id'],$col,$data_update,$tbname);
					}else{
						$tbname = 'users';
						$col = 'user_id';
						$data_update = array( 
	...
	public function start_student(){
		...
		if($this->input->post()){
			...
			$account = $this->model_base->get_one($user_name,$col,$table_name);
			$this->db->flush_cache();
